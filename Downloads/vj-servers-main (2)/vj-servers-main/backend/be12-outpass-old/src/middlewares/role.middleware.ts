  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        // Mentor row example: Mentor – S.Jhanavi ... mentor_email ... mentor_phone
        if (rowText.toLowerCase().includes('mentor')) {
          ...
          if (!mentorName || !mentorEmail || !isVnrEmail(mentorEmail)) {
            ...
            console.log(`⚠️ Row ${rowNumber}: Invalid mentor row`, row);
            currentMentor = null;
            continue;
          }
        ...
        // Student row: usually col1 = H.T.NO., col2 = Name of the Student
        let rollNumber = '';
        let studentName = '';
        ...
        if (isValidRollNumber(col1) && col2) {
          ...
          // Fallback: find roll number anywhere in row, then next useful text as name
        ...
        if (currentMentor) {
          try {
            ...
            console.log(`✅ Mapped ${studentEmail} to ${currentMentor.email}`);
          } catch (mappingError) {
