...
export function parseExcelFile(filePath: string): ExcelParseResult {
  try {
    ...
    const columnMappings = {
      ...
      name: ['name', 'student name', 'student_name', 'studentname', 'full name', 'name of the student'],
      rollNumber: ['roll', 'roll number', 'roll_number', 'rollnumber', 'reg no', 'registration', 'h.t.no.', 'h.t.no', 'htno'],
      ...
      mobile: ['student mobile no', 'student mobile', 'student_mobile', 'studentmobile', 'student mobile phone'],
      parentMobile: ['parent mobile no', 'parent mobile', 'parent_mobile', 'parentmobile', 'parent mobile phone'],
      mentorMobile: ['mentor phone', 'mentor mobile', 'mentor_mobile', 'mentor_phone']
    };
    ...
    if (columnIndices.email === undefined && columnIndices.rollNumber === undefined) {
      throw new Error('Email or Roll Number (H.T.NO.) column is required.');
    }
    ...
    for (let i = 1; i < rawData.length; i++) {
      ...
      const hasEmail = columnIndices.email !== undefined && row[columnIndices.email];
      const hasRoll = columnIndices.rollNumber !== undefined && row[columnIndices.rollNumber];
      
      if (!row || row.length === 0 || (!hasEmail && !hasRoll)) {
        continue; // Skip empty rows
      }
      
      try {
        let email = '';
        if (columnIndices.email !== undefined && row[columnIndices.email]) {
          email = String(row[columnIndices.email] || '').trim().toLowerCase();
        } else if (columnIndices.rollNumber !== undefined && row[columnIndices.rollNumber]) {
          const roll = String(row[columnIndices.rollNumber] || '').trim().toUpperCase();
          email = `${roll.toLowerCase()}@vnrvjiet.in`;
        }
        ...
        const studentData: ExcelStudentData = {
          ...
          mobile: columnIndices.mobile !== undefined ? String(row[columnIndices.mobile] || '').trim() : undefined,
          parentMobile: columnIndices.parentMobile !== undefined ? String(row[columnIndices.parentMobile] || '').trim() : undefined,
          mentorMobile: columnIndices.mentorMobile !== undefined ? String(row[columnIndices.mentorMobile] || '').trim() : undefined,
        };
